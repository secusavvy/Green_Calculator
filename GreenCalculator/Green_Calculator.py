...
class Calculator:
    def __init__(self,master):
        master.title("Green Calculator")
        master.geometry('357x420+0+0')
        master.config(bg='green4')
        master.resizable(False,False)

        self.equation = StringVar()
        self.entry_value = ''
        Entry(width=17,bg='DarkOliveGreen1',font=('Arial Bold',28),textvariable=self.equation).place(x=0,y=0)
        ...
        Button(width=11, height=4, text='=', relief='flat', bg= 'green2', command=self.solve).place(x=270, y=350)
        Button(width=11, height=4, text='C', relief='flat', bg= 'green2', command=self.clear).place(x=0, y=350)
